...
const RemoveMovie = () => {
  ...
  const handleSelectMovie = (title) => {
    if (selectedMovies.includes(title)) {
      setSelectedMovies(selectedMovies.filter((movieTitle) => movieTitle !== title));
    } else {
  ...
  const handleRemoveMovies = async () => {
    try {
      // Make a POST request
      const response = await fetch("http://192.168.10.101:5000/delete_movie", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          movie_name: selectedMovies[0]
        }),
      });
            ...
            <div className="movie-card" key={movie.id}>
              <label>
                <input
                  ...
                  onChange={() => handleSelectMovie(movie.title)}
                />
